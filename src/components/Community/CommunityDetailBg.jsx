...
function CommunityDetailBg() {
  ...
  const [comDetailDate, setComDetailDate] = useState(null);
    ...
    async function fetchData() {
      try {
        ...
        if (response.status === 200) {
          ...
          setComDetailDate(data.date);
        } else {
          console.error(
            'Error fetching community content:',
            response.statusText
          );
          setComDetailDate('2023.11.01 ~ 2023.11.11 21:00');
        }
      } catch (error) {
  ...
  const dummyData = [
    {
      ...
      id3Des:
        '이번 발표는 수도권을 겨냥한 국힘의 내년 총선 전략이라는 분석이 나와요. 김포시는 수도권에서도 표심이 왔다 갔다 하는 ‘스윙보트’ 지역이니“서울이랑 합칠게” 하면 표심을 꽉 잡을 수 있지 않겠냐는 것. 구리·광명·하남시 등 주변 지역 민심에도 영향을 줄 수 있고요. 이번 발표는 수도권을 겨냥한 국힘의 내년 총선 전략이라는 분석이 나와요. 김포시는 수도권에서도 표심이 왔다 갔다 하는 ‘스윙보트’ 지역이니“서울이랑 합칠게” 하면 표심을 꽉 잡을 수 있지 않겠냐는 것. 구리·광명·하남시 등 주변 지역 민심에도 영향을 줄 수 있고요.',
      date: '2023.11.01 ~ 2023.11.11 21:00',
    },
  ];
